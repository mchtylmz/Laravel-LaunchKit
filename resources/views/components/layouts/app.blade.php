        ...
        @auth
            ...
            <aside class="fixed inset-y-0 left-0 z-40 w-72 -translate-x-full border-r border-slate-200/70 bg-white/90 p-4 shadow-2xl shadow-slate-950/10 backdrop-blur-xl transition dark:border-white/10 dark:bg-slate-950/80 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 lg:shadow-none" :class="{ 'translate-x-0': sidebarOpen }">
                ...
                <nav class="mt-3 space-y-1">
                    ...
                    <a class="nav-link {{ request()->routeIs('api-tokens.*') ? 'nav-link-active' : '' }}" href="{{ route('api-tokens.index') }}"><span>API Tokens</span><span class="text-xs opacity-60">05</span></a>
                    @can('manage settings')
                        <a class="nav-link {{ request()->routeIs('settings.*') ? 'nav-link-active' : '' }}" href="{{ route('settings.edit') }}"><span>Settings</span><span class="text-xs opacity-60">06</span></a>
                    @endcan
                    @can('manage users')
                        <a class="nav-link {{ request()->routeIs('users.*') ? 'nav-link-active' : '' }}" href="{{ route('users.index') }}"><span>Users</span><span class="text-xs opacity-60">07</span></a>
                    @endcan
                    @can('manage roles')
                        <a class="nav-link {{ request()->routeIs('roles.*') ? 'nav-link-active' : '' }}" href="{{ route('roles.index') }}"><span>Roles</span><span class="text-xs opacity-60">08</span></a>
                    @endcan
                    @can('view activity logs')
                        <a class="nav-link {{ request()->routeIs('activity-logs.*') ? 'nav-link-active' : '' }}" href="{{ route('activity-logs.index') }}"><span>Activity logs</span><span class="text-xs opacity-60">09</span></a>
                    @endcan
                    @can('manage files')
